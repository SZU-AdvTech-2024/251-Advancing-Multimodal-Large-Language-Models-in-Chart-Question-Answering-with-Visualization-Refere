...
# 输出目录
OUTPUT_DIR="./playground/eval/chartqa/$(basename $CKPT)/RAG/$(date +%Y%m%d%H%M%S)"
mkdir -p $OUTPUT_DIR

# 定义数据集
declare -A QUESTION_FILES=(
    # ["aug"]="/home/lhy/work/front-tech/playground/data/chartqa/test_aug.jsonl"
    ["human"]="playground/data/chartqa/human_compositional.jsonl"
)
...
run_evaluation() {
    ...
    for IDX in $(seq 0 $((CHUNKS - 1))); do
        ...
        CUDA_VISIBLE_DEVICES=${GPULIST[$IDX]} python -m llava.eval.model_vqa_loader_RAG \
            --model-path $CKPT \
