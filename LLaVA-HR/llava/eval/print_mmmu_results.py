...
from llava.eval.mmmu_utils.data_utils import CAT_SHORT2LONG, DOMAIN_CAT2SUB_CAT

from llava.eval.mmmu_utils.eval_utils import calculate_ins_level_acc


def main():
    parser = ArgumentParser()
    parser.add_argument(
        "--path", type=str, default="./example_outputs/blip2_flant5xxl", help="The path to output directory."
    )
    ...
    table.append(
        ["Overall", np.sum([cat_results["num_example"] for cat_results in all_results.values()]), round(all_ins_acc, 3)]
    )
