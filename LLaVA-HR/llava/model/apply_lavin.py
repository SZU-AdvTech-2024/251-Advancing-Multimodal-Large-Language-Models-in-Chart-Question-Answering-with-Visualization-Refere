...
class Adapter_Router_plus(nn.Module):
    def __init__(self, config, lavin_in_features=512, enable_clip=False):
        super().__init__()
        if enable_clip:
            hidden_dim = 8
        else:
        ...
        self.conv_A = nn.ModuleList(
            [nn.Conv1d(lavin_in_features, hidden_dim, 1, groups=1, bias=True) for i in range(config.lavin_n_router)]
        )

        self.conv_B = nn.ModuleList(
            [
                nn.Conv1d(hidden_dim, lavin_in_features, 1, groups=config.lavin_groups, bias=True)
                for i in range(config.lavin_n_router)
            ]
        )
    ...
    def forward(self, x, weights=None):
        ...
        for i, conv in enumerate(self.conv_A):
            out += (
                self.conv_B[i](F.silu(self.dropout(self.conv_A[i](x.type(dst_type)))))
                * self.scale
                * weights[:, i, None, None]
            )
...
class LaVINDecoderLayer(LlamaDecoderLayer):
    def __init__(self, config: LlamaConfig, lavin_config):
        ...
        self.cache_weights = torch.zeros((100, 2)).cuda()
    ...
    def forward(
        ...
        # lavin adapter
        if self.training:
            weights = torch.softmax(self.adapter.expert_weights(hidden_states[:, 1]) / self.adapter.t, -1).type_as(
                residual
            )
        else:
            ...
            if past_key_value is None:
                ...
                self.cache_weights[: residual.shape[0]] = torch.softmax(
                    self.adapter.expert_weights(hidden_states[:, 1].type(dst_type)) / self.adapter.t, -1
                )
