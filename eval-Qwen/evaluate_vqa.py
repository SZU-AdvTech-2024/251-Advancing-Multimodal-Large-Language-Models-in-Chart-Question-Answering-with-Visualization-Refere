...
ds_collections = {
    ...
    "chartqa_test_augmented": {
        "train": "playground/data/chartqa/train_augmented.jsonl",
        "test": "playground/data/chartqa/test_augmented.jsonl",
        "metric": "relaxed_accuracy",
        "max_new_tokens": 100,
    },
    "gqa_testdev": {
    ...
    def __getitem__(self, idx):
        ...
        if self.few_shot > 0:
            ...
            for sample in few_shot_samples:
                ...
                few_shot_prompt += self.prompt.format(sample["image"], sample["question"]) + f" {sample['answer']}"
...
if __name__ == "__main__":
    ...
    parser.add_argument("--checkpoint", type=str, default="Qwen/Qwen-VL-Chat")
    parser.add_argument("--dataset", type=str, default="chartqa_test_augmented")
    ...
    if torch.distributed.get_rank() == 0:
        ...
        if ds_collections[args.dataset]["metric"] == "vqa_score":
            vqa = VQA(ds_collections[args.dataset]["annotation"], ds_collections[args.dataset]["question"])
            results = vqa.loadRes(resFile=results_file, quesFile=ds_collections[args.dataset]["question"])
            ...
            json.dump(merged_outputs, open(results_file, "w"), ensure_ascii=False)
            print(
                "python infographicsvqa_eval.py -g "
                + ds_collections[args.dataset]["annotation"]
                + " -s "
                + results_file
            )
            os.system(
